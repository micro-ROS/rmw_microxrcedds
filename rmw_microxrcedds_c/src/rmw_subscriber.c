...
rmw_subscription_t* create_subscriber(const rmw_node_t* node,
                                      ...
                                      bool ignore_local_publications)
{
    ...
    rmw_subscription_t* rmw_subscriber =
        (rmw_subscription_t*)rmw_allocate(sizeof(rmw_subscription_t));
    ...
    if (!rmw_subscriber->topic_name)
    {
        ...
        if (!memory_node)
        {
            ...
            // TODO(Borja) micro_xrcedds_id is duplicated in
            //  subscriber_id and in subscription_gid.data
            CustomSubscription* subscription_info = (CustomSubscription*)memory_node->data;
            subscription_info->subscriber_id =
                uxr_object_id(micro_node->id_gen++, UXR_SUBSCRIBER_ID);
            ...
            if (!subscription_info->type_support_callbacks)
            {
                ...
                memcpy(subscription_info->subscription_gid.data,
                       &subscription_info->subscriber_id,
                       sizeof(uxrObjectId));
                ...
                generate_name(&subscription_info->subscriber_id,
                              subscriber_name,
                ...
                subscriber_req =
                    uxr_buffer_create_subscriber_xml(&micro_node->session,
                ...
                topic_req =
                    uxr_buffer_create_topic_xml(&micro_node->session,
                                                   micro_node->reliable_output,
                ...
                topic_req =
                    uxr_buffer_create_topic_ref(&micro_node->session,
                                                micro_node->reliable_output,
                                                subscription_info->topic_id,
                                                micro_node->participant_id,
                                                profile_name,
...
#ifdef MICRO_XRCEDDS_USE_XML
                if (!build_datareader_xml(topic_name,
                                          subscription_info->type_support_callbacks,
                                          qos_policies, xml_buffer,
                                          sizeof(xml_buffer)))
                {
                ...
                datareader_req =
                    uxr_buffer_create_datareader_xml(&micro_node->session,
                                                        micro_node->reliable_output,
                                                        subscription_info->datareader_id,
                                                        subscription_info->subscriber_id,
                                                        xml_buffer,
                ...
                datareader_req =
                    uxr_buffer_create_datareader_ref(&micro_node->session,
                                                     micro_node->reliable_output,
                                                     subscription_info->datareader_id,
                                                     subscription_info->subscriber_id,
                                                     profile_name,
                ...
                if (!uxr_run_session_until_all_status(&micro_node->session,
                                                      1000,
                                                      requests,
                                                      status,
                                                      3))
...
rmw_ret_t rmw_destroy_subscription(rmw_node_t* node, rmw_subscription_t* subscription)
{
    ...
    if (!node)
    {
        ...
        int delete_datareader =
            uxr_buffer_delete_entity(&micro_node->session,
                                     micro_node->reliable_output,
