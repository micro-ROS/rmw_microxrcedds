  ...
  if (!rmw_subscriber->topic_name) {
    ...
    if (!memory_node) {
      ...
      if (!subscription_info->type_support_callbacks) {
        ...
        if (!build_topic_xml(topic_name, subscription_info->type_support_callbacks, qos_policies,
          xml_buffer,
          sizeof(xml_buffer)))
        {
          RMW_SET_ERROR_MSG("failed to generate xml request for subscriber creation");
...
#ifdef MICRO_XRCEDDS_USE_XML
        if (!build_datareader_xml(topic_name, subscription_info->type_support_callbacks,
