...
{
  if (!check_uxrce_rmw_identifier(node->implementation_identifier)) {
...
{
  if (!check_uxrce_rmw_identifier(publisher->implementation_identifier)) {
    RMW_SET_ERROR_MSG("node handle not from this implementation");
    return;
  }
...
{
  if (!check_uxrce_rmw_identifier(subscriber->implementation_identifier)) {
    RMW_SET_ERROR_MSG("node handle not from this implementation");
    return;
  }
