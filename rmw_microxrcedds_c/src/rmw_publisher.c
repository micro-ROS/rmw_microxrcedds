...
rmw_publisher_t* create_publisher(const rmw_node_t* node,
    ...
    if (!rmw_publisher->topic_name)
    {
        ...
        if (!memory_node)
        {
            ...
            publisher_info->publisher_id =
                uxr_object_id(micro_node->id_gen++, UXR_PUBLISHER_ID);
            ...
            if (publisher_info->type_support_callbacks == NULL)
            {
                ...
                memset(publisher_info->publisher_gid.data,
                       0,
                       RMW_GID_STORAGE_SIZE);
                memcpy(publisher_info->publisher_gid.data,
                       &publisher_info->publisher_id,
                       sizeof(uxrObjectId));
                ...
                generate_name(&publisher_info->publisher_id,
                    publisher_name,
                    sizeof(publisher_name));
                char xml_buffer[400];
                if (!build_publisher_xml(publisher_name,
                                         xml_buffer,
                                         sizeof(xml_buffer)))
                {
                ...
                publisher_req =
                    uxr_buffer_create_publisher_xml(publisher_info->session,
                                                       micro_node->reliable_output,
                                                       publisher_info->publisher_id,
                                                       micro_node->participant_id,
                                                       xml_buffer,
                                                       UXR_REPLACE);
#elif defined(MICRO_XRCEDDS_USE_REFS)
                // Publisher by reference does not make sense in current micro
                // XRCE-DDS implementation.
                publisher_req =
                    uxr_buffer_create_publisher_xml(publisher_info->session,
                                                       micro_node->reliable_output,
                                                       publisher_info->publisher_id,
                ...
                topic_req =
                    uxr_buffer_create_topic_xml(publisher_info->session,
                                                   micro_node->reliable_output,
                ...
                if (!build_topic_profile(topic_name,
                                         profile_name,
                                         sizeof(profile_name)))
                {
                ...
                topic_req =
                    uxr_buffer_create_topic_ref(publisher_info->session,
                                                micro_node->reliable_output,
                                                publisher_info->topic_id,
                                                micro_node->participant_id,
                                                profile_name,
                ...
                publisher_info->datawriter_id =
                    uxr_object_id(micro_node->id_gen++,
                                  UXR_DATAWRITER_ID);

                uint16_t datawriter_req;
#ifdef MICRO_XRCEDDS_USE_XML
                if (!build_datawriter_xml(topic_name,
                                          publisher_info->type_support_callbacks,
                ...
                datawriter_req =
                    uxr_buffer_create_datawriter_xml(publisher_info->session,
                                                        micro_node->reliable_output,
                                                        publisher_info->datawriter_id,
                                                        publisher_info->publisher_id,
                                                        xml_buffer,
                                                        UXR_REPLACE);
#elif defined(MICRO_XRCEDDS_USE_REFS)
                if (!build_datawriter_profile(topic_name,
                                              profile_name,
                                              sizeof(profile_name)))
                {
                    RMW_SET_ERROR_MSG("failed to generate xml request for node creation");
                ...
                datawriter_req =
                    uxr_buffer_create_datawriter_ref(publisher_info->session,
                                                     micro_node->reliable_output,
                                                     publisher_info->datawriter_id,
                                                     publisher_info->publisher_id,
                                                     profile_name,
                ...
                if (!uxr_run_session_until_all_status(publisher_info->session,
                                                      1000,
                                                      requests,
                                                      status,
                                                      3))
...
rmw_ret_t rmw_destroy_publisher(rmw_node_t* node, rmw_publisher_t* publisher)
{
    ...
    if (!node)
    {
        ...
        int delete_writer =
            uxr_buffer_delete_entity(publisher_info->session,
...
rmw_ret_t rmw_publish(const rmw_publisher_t* publisher, const void* ros_message)
{
    ...
    if (strcmp(publisher->implementation_identifier,
               rmw_get_implementation_identifier()) != 0)
    {
        RMW_SET_ERROR_MSG("publisher handle not from this implementation");
        ret = RMW_RET_ERROR;
                                     ...
                                     topic_length))
        {
            ...
            written &=
                uxr_run_session_until_confirm_delivery(publisher_info->session, 1000);
        }
        if (!written)
        {
