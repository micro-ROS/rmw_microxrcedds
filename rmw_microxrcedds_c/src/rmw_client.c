  ...
  const rmw_qos_profile_t * qos_policies)
{
  ...
  if (!node) {
    ...
    if (!run_xrce_session(custom_node->context, client_req, custom_node->context->creation_timeout)) {
  ...
  rmw_client_t * client)
{
  ...
  if (!node) {
    ...
    if (!run_xrce_session(custom_node->context, delete_client, custom_node->context->destroy_timeout)) {
