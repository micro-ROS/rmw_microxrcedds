...
int main(int argc, char * argv[])
{
  ...
  if (argc < 3) 
  {
    printf("Usage: ros2 run rcl_example_publisher rcl_example_publisher [agent ip] [agent port]\n");
    exit(0);
  }
  ...
  rmw_init_options_impl_t uxrceaddress;
  strcpy(uxrceaddress.agent_address, argv[1]);
  uxrceaddress.agent_port = atoi(argv[2]);
  rmw_init_options_t * rmw_init_options = rcl_init_options_get_rmw_init_options(&init_options);
  rmw_init_options->impl = &uxrceaddress;
  ...
  ret = rcl_node_init(&node, "vector3_publisher", "", &context, &node_ops);
  ...
  ret = rcl_publisher_init(&publisher_vector3, &node, pub_type_support, topic_name, &pub_opt);
  ...
  ret = rcl_wait_set_init(&wait_set, 0, 0, 0, 0, 1, rcl_get_default_allocator());
  ...
  {     
    ret = rcl_publish( &publisher_vector3, (const void *) &topic_data);
